...
"""RecallGuard V2: permissionless, evidence-bound marketplace recall checks."""
...
class RecallGuard(gl.Contract):
    ...
    authorized_listing_evidence_domains: DynArray[str]
    ...
    def __init__(
        ...
        self._load_domains(authorized_listing_evidence_domains, self.authorized_listing_evidence_domains)
    ...
    def _load_domains(self, domains: DynArray[str], destination: DynArray[str]):
        if len(domains) == 0:
            self._fail("EVIDENCE_INTEGRITY:EMPTY_DOMAIN_ALLOWLIST")
    ...
    def _canonical_listing_identity(
        self,
        marketplace_host: str,
        external_listing_id: str,
        product_id: str,
...
"""
    ...
    def register_listing(
        ...
        canonical_evidence_url = self._canonical_https_url(evidence_url)
        self._validate_sha256(evidence_sha256)

        if self._host(canonical_listing_url) != normalized_marketplace_host:
            self._fail("EVIDENCE_INTEGRITY:LISTING_HOST_MISMATCH")
        if not self._is_authorized_host(canonical_listing_url, self.authorized_marketplace_domains):
            self._fail("EVIDENCE_INTEGRITY:WRONG_MARKETPLACE_DOMAIN")
        if not self._is_authorized_host(canonical_evidence_url, self.authorized_listing_evidence_domains):
            self._fail("EVIDENCE_INTEGRITY:WRONG_LISTING_SOURCE_DOMAIN")
