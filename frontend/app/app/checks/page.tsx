...
export default function ChecksPage() {
  ...
  useEffect(() => {
    ...
    const fromUrl = params.get("listingId");
  ...
  async function hashRecall() {
    ...
    } catch (nextError) { setHashMessage(`Could not calculate automatically: ${String(nextError).replace(/^Error:\s*/i, "")}`); }
  ...
  async function submit(event: FormEvent) {
    ...
    if (!wallet) { setErrorState(new Error("Connect a wallet before requesting an assessment")); return; }
    ...
    setProgress({ stage: "PRECONDITION_READ", detail: "Reading the listing before submission" });
    try {
      const result = await contract.requestAssessment({ listingId, recallUrl, recallSha256, walletAddress: wallet }, setProgress);
      router.push(`/app/checks/${result.assessmentId}`);
    } catch (nextError) {
      setErrorState(nextError);
  ...
  return <div className="page-stack">
    <section className="page-intro"><div><div className="eyebrow">Decision workflow</div><h2>Recall checks</h2><p>Ask the Intelligent Contract to evaluate one tracked listing against one authoritative recall notice. The contract owns the verdict; this screen only prepares, submits, and reconciles the transaction.</p></div><Link className="button button-secondary" href="/app/listings"><Icon name="box" size={16} />View listings</Link></section>
