...
const initial = { productId: "", productName: "", manufacturer: "", model: "", serialOrLot: "", listingUrl: "", evidenceUrl: "", evidenceSha256: "" };
...
export default function NewListingPage() {
  ...
  return <div className="page-stack narrow-page">
    ...
    <form className="surface form-surface" onSubmit={(event) => void submit(event)}>
      <div className="form-section"><div className="form-section-title"><span className="section-number">01</span><div><h3>Product identity</h3><p>Use the identity your team uses to track this product or lot.</p></div></div><div className="form-grid"><Field label="Product or listing name" required value={form.productName} onChange={(value) => update("productName", value)} placeholder="Example Pump XP-100" /><Field label="Product ID" required value={form.productId} onChange={(value) => update("productId", value)} placeholder="Internal SKU or product code" /><Field label="Manufacturer" required value={form.manufacturer} onChange={(value) => update("manufacturer", value)} placeholder="Manufacturer name" /><Field label="Model" required value={form.model} onChange={(value) => update("model", value)} placeholder="Model or family" /><Field label="Serial or lot" required value={form.serialOrLot} onChange={(value) => update("serialOrLot", value)} placeholder="Serial range, lot, or batch" /></div></div>
      ...
      <div className="form-footer"><div><strong>Ready to register?</strong><span>This creates an ACTIVE listing record after finalized execution.</span></div><button className="button button-primary button-large" type="submit" disabled={Boolean(progress && progress.stage !== "FAILED")}>{progress && progress.stage !== "FAILED" ? "Waiting for finality..." : "Register listing"}<Icon name="arrow" size={16} /></button></div>
    </form>
    {info && <TechnicalDetails><div className="detail-grid"><span>Configured authorities</span><code>{info.authorized_recall_domains.join(" · ")}</code><span>Maximum evidence</span><code>{info.max_evidence_bytes.toLocaleString()} bytes</code><span>Source model</span><code>{info.authoritative_source_semantics}</code></div></TechnicalDetails>}
  </div>;
}
