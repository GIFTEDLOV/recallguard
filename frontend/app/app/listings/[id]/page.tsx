...
export default function ListingDetailPage() {
  ...
  return <div className="page-stack">
    ...
    <div className="content-grid detail-grid-layout">
      <section className="surface section-card"><div className="section-heading"><div><div className="eyebrow">Canonical identity</div><h3>What this record is bound to</h3></div></div><dl className="property-list"><Property label="Identity version" value={listing.identity_version} /><Property label="Marketplace host" value={listing.marketplace_host} /><Property label="External listing ID" value={listing.external_listing_id} /></dl><p className="field-help">This stable ID follows the marketplace namespace and external listing ID. Product description, metadata, URL representation, and evidence snapshots do not create a new identity.</p><dl className="property-list"><Property label="Product ID" value={listing.product_id} /><Property label="Manufacturer" value={listing.manufacturer} /><Property label="Model" value={listing.model} /><Property label="Serial / lot" value={listing.serial_or_lot || "Not supplied"} /></dl><div className="source-card"><div className="source-card-icon"><Icon name="link" size={17} /></div><div><span>Marketplace listing</span><strong>{hostFromUrl(listing.listing_url)}</strong><a href={listing.listing_url} target="_blank" rel="noreferrer">Open public URL <Icon name="external" size={13} /></a></div></div><div className="source-card"><div className="source-card-icon"><Icon name="shield" size={17} /></div><div><span>Mutable evidence snapshot</span><strong>{hostFromUrl(listing.evidence_url)}</strong><a href={listing.evidence_url} target="_blank" rel="noreferrer">Open evidence source <Icon name="external" size={13} /></a></div><code>{listing.evidence_sha256}</code></div></section>
      <section className="surface section-card"><div className="section-heading"><div><div className="eyebrow">Authoritative state</div><h3>{latest ? "Consensus history exists" : "Awaiting first assessment"}</h3></div></div>{latest ? <div className="latest-outcome"><div className="outcome-badges"><StateBadge state={listing.state} /><VerdictBadge verdict={latest.verdict} /></div><p>{listing.state === "BLOCKED" ? "At least one recorded relevant assessment is affected. A later favorable notice cannot clear this listing." : listing.state === "REVIEW_REQUIRED" ? "At least one recorded relevant assessment is inconclusive. A favorable unrelated notice cannot erase that unresolved result." : "All recorded relevant assessments are not affected."}</p><Link className="text-link" href={`/app/assessments/${latest.id}`}>Open latest assessment <Icon name="arrow" size={14} /></Link></div> : <EmptyState icon="scan" title="Not yet assessed" body="Registration only records the listing. It never produces a cleared or safe state." action={<Link className="button button-primary" href={`/app/listings/${listing.id}/check`}><Icon name="scan" size={15} />Report recall / Challenge listing</Link>} />}</section>
    </div>
    <section className="surface section-card"><div className="section-heading"><div><div className="eyebrow">Append-only history</div><h3>Assessment history</h3></div><span className="section-count">{history.length} recorded assessment{history.length === 1 ? "" : "s"}</span></div>{history.length === 0 ? <div className="quiet-empty">No consensus-backed assessment exists yet. Any connected user can start the check.</div> : <div className="history-list">{history.map((assessment) => <Link className="history-row" href={`/app/assessments/${assessment.id}`} key={assessment.id}><div className="history-marker"><Icon name="shield" size={15} /></div><div className="history-main"><strong>{labelForState(assessment.state_after)} after {assessment.verdict.replace("_", " ").toLowerCase()}</strong><span>Requester {shortHash(assessment.requested_by, 8, 6)} · authority {hostFromUrl(assessment.recall_url)} · notice {shortHash(assessment.notice_id, 8, 6)} · snapshot {shortHash(assessment.snapshot_id, 8, 6)} · SHA {shortHash(assessment.recall_sha256, 8, 6)} · recorded</span></div><div className="history-right"><VerdictBadge verdict={assessment.verdict} /><StateBadge state={assessment.state_after} /><Icon name="arrow" size={15} /></div></Link>)}</div>}</section>
