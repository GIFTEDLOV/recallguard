...
export function StateBadge({ state, technical = false }: { state: ListingState; technical?: boolean }) {
  return <span className={`state-badge state-${state.toLowerCase()}`} aria-label={`Listing state: ${labelForState(state)}`}><span className="state-dot" />{technical ? state : labelForState(state)}</span>;
}
